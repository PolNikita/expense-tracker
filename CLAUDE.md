...
```bash
pnpm typecheck   # нет TS-ошибок
pnpm lint        # нет lint-ошибок
```

Убедись, что ветка не отстала от `main`:
...
```bash
gh pr create \
  --title "feat(web): add main screen" \
  --body "$(cat <<'EOF'
## Что сделано
- ...
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | /expenses?limit=&offset= | постраничный список расходов |

## Как тестировать
- [ ] ...

## Скриншоты
<!-- прикрепи, если есть UI-изменения -->
EOF
)"
```

Параметры:
- `--base main` — если надо явно указать целевую ветку (по умолчанию и так `main`)
- `--draft` — если PR ещё не готов к ревью
